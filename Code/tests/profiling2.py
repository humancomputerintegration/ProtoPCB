...
def get_num_components():
	...
	#PCB 1
	#filename_net = current_directory + '/testfiles/SparkX_smol_RTC.net'

	#PCB 3
	#filename_net = current_directory + '/testfiles/Adafruit LSM9DS1 Rev C.net'

	#PCB 6
	#filename_net = current_directory + '/testfiles/Adafruit AHT20 Temperature & Humidity.net'
...
def get_num_nets():
	...
	# PCB
	#filename_net = current_directory + '/testfiles/UNO-TH_Rev3e.net'
	...
	for net in net_arr:
		for node in net['node arr']:
			if node['ref'] not in refs_touched:
				total_pins += node['total pins']
				refs_touched.append(node['ref'])

	print('total pins')
	print(total_pins)

	print('componets')
	print(len(refs_touched))
